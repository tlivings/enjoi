...
const enjoi = function (schema, options) {
    ...
    Assert.ok(Util.isObject(schema) || Util.isString(schema), 'Expected schema to be an object or type string.');
    Assert.ok(Util.isObject(options), 'Expected options to be an object.');

    const subSchemas = options.subSchemas;
    const types = options.types;
    const refineType = options.refineType;

    Assert.ok(!subSchemas || Util.isObject(subSchemas), 'Expected options.subSchemas to be an object.');
    Assert.ok(!types || Util.isObject(types), 'Expected options.types to be an object.');
    Assert.ok(!refineType || Util.isFunction(refineType), 'Expected options.refineType to be a function.');
    ...
    function resolveAnyOf(current) {
        Assert.ok(Util.isArray(current.anyOf), 'Expected anyOf to be an array.');
    ...
    function resolveAllOf(current) {
        Assert.ok(Util.isArray(current.allOf), 'Expected allOf to be an array.');
    ...
    function resolveOneOf(current) {
        Assert.ok(Util.isArray(current.oneOf), 'Expected allOf to be an array.');
