...
module.exports = function enjoi(schema, options) {
    ...
    function number(current) {
        var joischema = Joi.number().options({ language: { number: current.messages } });
        ...
        if (current.label) {
            joischema = joischema.label(current.label);
        }

        if (current.greater || current.greater === 0) {
	        joischema = joischema.greater(current.greater);
        }
    ...
    function regularString(current) {
        var joischema = Joi.string().options({ language: { any: current.messages, string: current.messages } });
        current.regex && (joischema = joischema.regex(new RegExp(current.regex.pattern, current.regex.options), current.regex.patternName));
    ...
    function email(current) {
        var joischema = Joi.string().email().options({ language: { any: current.messages, string: current.messages } });
    ...
    function date(current) {
        var joischema = Joi.date().options({ language: { date: current.messages } });
