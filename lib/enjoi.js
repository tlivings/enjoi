...
module.exports = function enjoi(schema, options) {
    ...
    function resolveref(value) {
        ...
        if (id && subSchemas) {
            ...
            /*
             * Start of patching current implementation to include URI $ref resolution.
             * If we cannot find a reference within schema we assume it's external
             */
            if (!refschema && id !== '#') {
              try {        
                filedata = fs.readFileSync(__dirname + '/' + (id[id.length - 1] === '#' ? id.replace('#', '.json') : ''), {encoding: 'utf8'});
                refschema = JSON.parse(filedata);
              } catch (e) {
                throw e;
              }        
            }
        ...
        /*
         * Part 2 of adding URI to $ref, we need to take into account paths.
         */
        var i = 0;
        fragment = refschema;
        var path;
        
        while (typeof (path = paths[++i]) !== 'undefined') {
            fragment = (typeof fragment === 'object' && fragment[path]);
        }
