...
class SchemaResolver {
    constructor(root, { subSchemas, types, refineType, strictMode, extensions = [] }) {
        ...
        this.joi = Joi.extend(
          ...
          {
              type: 'allOf',
              base: Joi.array().items(Joi.object()).required(),
              coerce: {
                  from: 'string',
                  method(value) {
                      if (typeof value !== 'string' || (value[0] !== '[' && !/^\s*\[/.test(value))) {
              ...
              validate(value, helpers) {
                  // let error;
                  // helpers = Hoek.applyToDefaults(helpers || {}, {allowUnknown: true});
                  //
                  // for (const schema of value.items) {
                  //     schema.validate(value, helpers, (err) => {
                  //         error = err;
                  //     });
                  //     if (error) {
                  //         const details = error.details[0];
                  //         return this.createOverrideError(details.type, details.context, {path: details.path}, helpers, details.message);
                  //     }
                  // }
                  return { value };
              }
    ...
    resolveAllOf(schema) {
        ...
        return this.joi.allOf().items(...schema.allOf.map((schema) => this.resolve(schema)));
    }
